...
base_start      = 1980              # start year
base_end        = base_start + 30-1 # end year

subset = subset(precip,  (Ensemble==ens_target)  &
                        ((year>=base_start) & (year<=base_end)   )    
                )
...
To implement.  Let's make an array.  We will copy this over and over.

```{r}


rcp45_base_period_return  = array(0,c(n_ensembles,
                                    n_return_intervals))


rcp45_base_period_ci_ub   = array(0,c(n_ensembles,
                                    n_return_intervals))


rcp45_base_period_ci_lb   = array(0,c(n_ensembles,
                                    n_return_intervals))




rcp85_base_period_return  = array(0,c(n_ensembles,
                                    n_return_intervals))


rcp85_base_period_ci_ub   = array(0,c(n_ensembles,
                                    n_return_intervals))


rcp85_base_period_ci_lb   = array(0,c(n_ensembles,
                                    n_return_intervals))
...
for (ens_target in 1:n_ensembles) {
  ...
  # create segments for RCP 45 and RCP 85 for your period(s)
   ...
   subset_base = subset(precip,  (Ensemble==ens_target)  &
                                 ((year>=base_start) & (year<=base_end)   )
                        )
 

   fit_GP_45_base = fevd(x          = RCP45,
                         data       = subset_base,
                         units      = "mm",
                         time.units = "days",
                         threshold  = 2.0,
      ...
      return_GP = return.level(x             = fit_GP_45_base,
                         return.period = return_intervals,
                         do.ci         = TRUE)
      ...
      return_GP = return.level(x             = fit_GP_85_base,
                         return.period = return_intervals,
                         do.ci         = TRUE)
...
Plot return output
```{r}
plot(x = return_intervals,
     y = rcp85_base_period_return[1,], col='red')
lines(x = return_intervals,
     y = rcp85_base_period_ci_lb[1,], col='red')
lines(x = return_intervals,
     y = rcp85_base_period_ci_ub[1,], col='red')

points(x = return_intervals,
     y = rcp45_base_period_return[1,], col='blue')

lines(x = return_intervals,
     y = rcp45_base_period_ci_lb[1,], col='blue')

lines(x = return_intervals,
     y = rcp45_base_period_ci_ub[1,], col='blue')
