...
plot_var =  t_2m.45[,,1,30 ] - 273.17

filled.contour(x = lon, 
               y = lat,
               z = plot_var, 
               color.palette = colorRampPalette(c("violet","blue", "lightblue", "yellow", "orange", "red")),
               xlab = "Longitude", 
               ylab = "Latitude",
...
plot(x = time_months.45,
     y = t_2m.45[target_x,target_y,1,],
     main = paste("RCP 4.5 Air temp for Ensemble",
                  1, "in deg C"),
     type = "l",
     xlab = "Time",
     ylab = "Temperature")
